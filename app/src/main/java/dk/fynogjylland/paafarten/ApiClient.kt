...
fun ApiClient.createAbsence(baseUrl:String,token:String,type:String,from:String,to:String,note:String,callback:(Result<Unit>)->Unit){
    Thread {
        val r=runCatching {
            val root=if(baseUrl.endsWith("/")) baseUrl else "$baseUrl/"
            val conn=(URL(root+"absence.php").openConnection() as HttpURLConnection).apply{
            ...
            val body=listOf("type" to type,"date_from" to from,"date_to" to to,"note" to note)
                .joinToString("&"){(k,v)->URLEncoder.encode(k,"UTF-8")+"="+URLEncoder.encode(v,"UTF-8")}
            conn.outputStream.use{it.write(body.toByteArray(Charsets.UTF_8))}
            val code=conn.responseCode
            val raw=(if(code in 200..299)conn.inputStream else conn.errorStream)?.bufferedReader()?.use{it.readText()}.orEmpty()
