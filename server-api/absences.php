...
try{
 $q=db()->prepare("SELECT id,type AS kind,date_from AS `from`,date_to AS `to`,note,status,created_at FROM pf_absence_requests WHERE driver_local_id=? AND date_from<=? AND date_to>=? ORDER BY date_from");
